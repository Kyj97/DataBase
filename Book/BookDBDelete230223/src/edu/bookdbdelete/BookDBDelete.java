...
public class BookDBDelete {
	public static void main(String[] args) {
		...
		try {
			...
			conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/bookdb", "root","datatest"); // JDBC 연결
			System.out.println("DB 연결 완료");
			stmt = conn.createStatement(); // SQL문 처리용 Statement 객체 생성
			stmt.executeUpdate("delete from book where title='Pride & Prejudice' and author='제인 오스틴'"); // 레코드 삭제
			printTable(stmt);
